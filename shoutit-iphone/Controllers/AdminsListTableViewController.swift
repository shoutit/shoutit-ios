...
private extension AdminsListTableViewController {
    ...
    func handleProfileTapped(_ profile: Profile) {
        if case .page(let pageUser, _)? = Account.sharedInstance.loginState, profile.id == pageUser.id {
            showProfile(profile)
        } else {
    ...
    func showActionSheetWithAdminProfile(_ profile: Profile) {
