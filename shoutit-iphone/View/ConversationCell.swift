...
protocol MessagePresenting {
    func bindWithMessage(_ message: Message, previousMessage: Message?)
}
...
struct ConversationCellIdentifier {
    ...
    static let special = "SpecialMessageCell"
}
...
extension ConversationCell where Self: UIView {
    ...
    func addAvatarButtonToAvatarImageView() {
        ...
        avatarImageView?.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|[button]|", options: [], metrics: nil, views: views))
        avatarImageView?.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|[button]|", options: [], metrics: nil, views: views))
    }
    
    func hydrateAvatarImageView(_ imageView: UIImageView, withAvatarPath path: String?) {
        if let imagePath = path, let imgUrl = URL(string: imagePath) {
