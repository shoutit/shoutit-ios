    ...
    */
    ...
    func loadFetchResults() {
        PHPhotoLibrary.requestAuthorization({ (status) -> Void in
            ...
            case .authorized:
                ...
                if let fetchedAssetCollection = PHAssetCollection.fetchAssetCollections(with: .smartAlbum, subtype: .smartAlbumRecentlyAdded, options: nil).lastObject as? PHAssetCollection {
    ...
    func updateCachedAssets() {
        ...
        if self.collectionView != nil && self.assetsFetchResults?.count > 0 {
            ...
            if let assets = self.assetsFetchResults?.objects(at: IndexSet(integersIn: NSRange(location: 0, length: min(self.assetsFetchResults?.count ?? 0, self.maxFetchResultsCount)).toRange() ?? 0..<0)) as? Array<PHAsset> {
    ...
    */
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        default:
            ...
            self.addPhotoAction.setValue(NSString.localizedStringWithFormat(NSLocalizedString("Add %d Photos", tableName: "Nemo", bundle: Bundle.main, comment: "") as NSString, selectedItemsCount), forKey: "title")
        }
    }
